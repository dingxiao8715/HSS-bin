...
sub content_stat{     #right version; latest 2012.6.1
    my $seq=shift;
    my %kmer_num;
...
sub related_pro_stat{
    my $seq=shift;
    my %kmer=count_k_freq($seq,$kr);
    my %kmer_head=count_k_freq($seq,$krh);
    my %kmer_tail=count_k_freq($seq,$krt);
...
sub bbc_stat{         #right version; latest 2012.6.1
    my $seq=shift;
    my %kmer=count_k_freq($seq,$kb);
    my %kmer_head=count_k_freq($seq,$kbh);
    my %kmer_tail=count_k_freq($seq,$kbt);
    my %out;
